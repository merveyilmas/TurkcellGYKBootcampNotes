...
np.random.seed(42)
def generateDataWithLabelNoise(m=2000, noise_ratio=0.1):
    data=[]
    for _ in range(m):
        ...
        # %10 oranında hata ekledik
        if np.random.rand() < noise_ratio:
            approved = 1 - approved  # etiketi tersine çevirdi
...
df1 = generateDataWithLabelNoise()
